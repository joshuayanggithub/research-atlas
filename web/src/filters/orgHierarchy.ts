...
/** Build the curated root org -> child unit tree (roots keep insertion order). */
export function buildOrgTree(ds: Dataset): OrgNode[] {
  const insts = ds.orgs.institutions;
  return rootOrgKeys(ds).map((key) => ({
    key,
    inst: insts[key],
    children: insts[key].children
      .filter((c) => insts[c])
      .map((c) => ({ key: c, inst: insts[c], children: [] })),
  }));
}
...
export function topAuthorsInNodes(
  ...
  for (const a of ds.authors) nameOf.set(a.authorId, a.name);
