"""Append the 1991-2014 backfill to the live corpus: 912,429 -> ~1,000,490 papers.
...
BACKUP = LIVE / "_pre_backfill_backup"
CORPUS_IN = LIVE / "corpus_backfill_1991_2014.parquet"
VECTORS_IN = LIVE / "embeddings_backfill_1991_2014.npy"
...
def main() -> None:
    ...
    log(f"live {live.height:,} + backfill {add.height:,} = {live.height + add.height:,}")
    ...
    log(f"filling {len(missing)} columns absent from the backfill: {missing[:8]}"
    ...
    for col in live.columns:
        ...
        if ta == pl.Null:  # live side empty -> adopt the backfill's real type
            ...
            casts.append(f"{col}: backfill {tb} -> {ta}")
    for c in casts:
        log(f"    {c}")
    ...
    meta.update({"n": int(out.shape[0]), "merged_from": ["2025-2026", "2015-2024", "1991-2014"]})
